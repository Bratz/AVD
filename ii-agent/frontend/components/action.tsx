...
const Action = ({ workspaceInfo, type, value, onClick }: ActionProps) => {
  ...
  const step_icon = useMemo(() => {
    const className = "h-4 w-4 text-foreground/70 flex-shrink-0 mt-[2px]";
    switch (type) {
      ...
      case TOOL.BROWSER_OPEN_NEW_TAB:
        return <Globe className={className} />;
      case TOOL.LIST_BANKING_APIS:
      case TOOL.GET_API_STRUCTURE:
      case TOOL.INVOKE_BANKING_API:
  ...
  const step_title = useMemo(() => {
    switch (type) {
      ...
      case TOOL.LIST_BANKING_APIS:
        return "Listing Banking APIs";
      case TOOL.GET_API_STRUCTURE:
        return "Getting API Structure";
      case TOOL.INVOKE_BANKING_API:
        return "Invoking Banking API";
      default:
        return type;
    }
  ...
  const step_value = useMemo(() => {
    switch (type) {
      ...
      case TOOL.LIST_BANKING_APIS:
        return value.tool_input?.category || "All APIs";
      case TOOL.GET_API_STRUCTURE:
        return value.tool_input?.api_name;
      case TOOL.INVOKE_BANKING_API:
        return value.tool_input?.api_name;
  ...
  return (
    <div
      onClick={onClick}
      className={`group cursor-pointer flex items-start gap-2 px-3 py-2 
    ...
    >
      ...
      <div className="flex flex-col gap-1.5 text-sm">
        <span className="text-foreground font-medium group-hover:text-foreground">
          {step_title}
        </span>
        <span className="text-muted-foreground font-medium truncate group-hover:text-foreground/80">
          {step_value}
        </span>
      </div>
    </div>
