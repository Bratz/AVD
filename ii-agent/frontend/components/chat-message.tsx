...
import { Check, CircleStop, Pencil, Folder,Zap,Brain } from "lucide-react";
...
const ChatMessage = ({
  ...
  // const isCopilot = window.location.pathname.includes('copilot');
  const isCopilot = typeof window !== 'undefined' && window.location.pathname.includes('copilot');
  ...
  const togglePlanCollapse = (messageId: string) => {
    ...
    <div className="col-span-4">
      ...
      >
          ...
          >
                ...
                {(() => {
                  ...
                  return filesToDisplay.map((fileName, fileIndex) => {
                    ...
                    if (isFolderMatch) {
                      ...
                      return (
                        <div
                          ...
                          className="inline-block ml-auto bg-secondary text-secondary-foreground rounded-2xl px-4 py-3 border border-border shadow-sm"
                        >
                          <div className="flex items-center gap-3">
                            <div className="flex items-center justify-center w-12 h-12 bg-blue-600 rounded-xl">
                              <Folder className="size-6 text-foreground" />
                            </div>
                    ...
                    return (
                      <div
                        key={`${message.id}-file-${fileIndex}`}
                        className="inline-block ml-auto bg-card text-foreground rounded-2xl px-4 py-3 border border-gray-700 shadow-sm"
                      >
                        <div className="flex items-center gap-3">
                          <div
            ...
            {message.content && (
              <motion.div
                className={`inline-block text-left rounded-lg ${
                  message.role === "user"
                    ? "bg-secondary p-3 max-w-[80%] text-secondary-foreground border border-border shadow-sm whitespace-pre-wrap"
                    : "text-foreground"
                } ${
              ...
              >
                {message.role === "user" ? (
                  <div>
                    {state.editingMessage?.id === message.id ? (
                      ...
                      <div className="relative group">
                        <div className="text-left">{cleanContent(message.content)}</div>
                  ...
                  <MessageContent isCopilot={isCopilot}>{cleanContent(message.content || '')}</MessageContent>
                )}
              </motion.div>
            )}
            {message.metadata && message.metadata.model_used && (
              ...
              >
                <div className="flex items-center gap-1.5 px-2 py-1 rounded-full bg-muted/50">
                  {message.metadata.model_used.includes("Nemotron") || 
                  message.metadata.model_used === state.toolSettings.simple_model ? (
                  ...
                  {message.metadata.complexity_score !== undefined && (
                    <span className="ml-1 opacity-70">
                      ({(message.metadata.complexity_score * 100).toFixed(0)}%)
                    </span>
                  )}
                </div>
              </motion.div>
            ...
            {message.plan && (
              ...
              >
                <PlanDisplay 
                  plan={message.plan} 
                  isCollapsed={collapsedPlans.has(message.id)}
                  onToggle={() => togglePlanCollapse(message.id)}
                />
              </motion.div>
            )}
        ...
        {state.isLoading && (
          ...
          >
            <motion.div
              className="inline-block p-3 text-left rounded-lg bg-muted/90 text-muted-foreground backdrop-blur-sm"
              initial={{ scale: 0.95 }}
              animate={{ scale: 1 }}
              transition={{
                type: "spring",
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="w-2 h-2 bg-foreground rounded-full animate-[dot-bounce_1.2s_ease-in-out_infinite_0ms]" />
                <div className="w-2 h-2 bg-foreground rounded-full animate-[dot-bounce_1.2s_ease-in-out_infinite_200ms]" />
                <div className="w-2 h-2 bg-foreground rounded-full animate-[dot-bounce_1.2s_ease-in-out_infinite_400ms]" />
                </div>
              </div>
            </motion.div>
          </motion.div>
        ...
        {state.isCompleted && (
          <div className="flex gap-x-2 items-center bg-[#25BA3B1E] text-green-600 text-sm p-2 rounded-full">
            ...
            <span>{brandingConfig.messages.taskCompleted}</span>
          </div>
        )}
        ...
        {state.isStopped && (
          <div className="flex gap-x-2 items-center bg-[#ffbf361f] text-yellow-300 text-sm p-2 rounded-full">
            ...
            <span>{brandingConfig.messages.taskStopped}</span>
          </div>
        )}
