...
from src.ii_agent.llm.base import LLMClient, TextResult, ToolCallParameters
...
from src.ii_agent.tools.utils import encode_image
from src.ii_agent.db.manager import DatabaseManager
from src.ii_agent.tools import AgentToolManager
from src.ii_agent.utils.constants import COMPLETE_MESSAGE
...
"""
    ...
    def run_impl(
        ...
        while remaining_turns > 0:
            ...
            if len(pending_tool_calls) == 0:
                ...
                response_text = self.history.get_last_assistant_text_response()
                self.logger_for_agent_logs.info(f"Sending final response: {response_text[:100]}...")
                self.message_queue.put_nowait(
                    RealtimeEvent(
                        type=EventType.AGENT_RESPONSE,
                        content={"text": response_text or "Task Completed"},  # Send actual response
                    )
                )
                return ToolImplOutput(
            ...
            if len(text_results) > 0:
                ...
                # Add this to emit thinking event
            ...
            # Handle tool calls
            if tool_call.tool_name == "sequential_thinking":
                # For sequential thinking, emit the thought content
                thought = tool_call.tool_input.get("thought", "")
                if thought:
                    self.message_queue.put_nowait(
                        RealtimeEvent(
                            type=EventType.AGENT_THINKING,
                ...
                # For other tools, emit normal tool call
                self.message_queue.put_nowait(
                    RealtimeEvent(
                        type=EventType.TOOL_CALL,
                        content={
    ...
    def run_agent(
        ...
        # # Add this to ensure final response is emitted
        # if result and not self.interrupted:
        #     self.message_queue.put_nowait(
        #         RealtimeEvent(
        #             type=EventType.AGENT_RESPONSE,
        #             content={"text": result},
    ...
    def add_tool_call_result(self, tool_call: ToolCallParameters, tool_result: str):
        ...
        self.history.add_tool_call_result(tool_call, tool_result)
            ...
            # Skip emitting tool result for sequential thinking (already shown as thinking)
        if tool_call.tool_name == "sequential_thinking":
            return
