...
from src.ii_agent.utils.constants import SONNET_4


class QueryComplexityClassifier:
    """BERT-based query complexity classifier"""
    ...
    def calculate_complexity(self, query: str) -> float:
        """Calculate query complexity score using BERT"""
        ...
        # Assuming index 1 is "complex" class
        complexity_prob = softmax_scores[1] if len(softmax_scores) > 1 else 0.5
        
        return float(complexity_prob)
    
    def get_banking_adjusted_complexity(self, query: str) -> float:
        """Adjust complexity based on banking-specific patterns"""
        base_complexity = self.calculate_complexity(query)
        ...
        simple_patterns = [
            "get customer", "show account", "balance", "customer id",
            "account details", "customer details", "profile"
        ]
        
        # Complex banking operations
        complex_patterns = [
            "analyze", "predict", "recommend", "compare", "trend",
            "risk assessment", "portfolio", "investment strategy"
        ]
...
class ChutesOpenAIClient(LLMClient):
    """Use Chutes models via OpenAI-compatible API with intelligent routing."""
    ...
    def __init__(
        ...
        """Initialize the Chutes OpenAI-compatible client with routing.
        
        Args:
            model_name: The default model name to use.
            simple_model: Model to use for simple queries (fast)
            complex_model: Model to use for complex queries (smart)
            complexity_threshold: Threshold for routing (0-1)
            use_routing: Whether to use intelligent routing
            bert_model: BERT model for complexity classification
        ...
        self.simple_model = simple_model or "NVIDIA/Nemotron-4-340B-Chat"  # Fast default
        self.complex_model = complex_model or model_name  # Smart default
        
        # Routing configuration
        self.use_routing = use_routing
        self.complexity_threshold = complexity_threshold
        
        # Initialize complexity classifier if routing is enabled
        if self.use_routing:
            try:
                self.classifier = QueryComplexityClassifier(bert_model)
                logging.info(f"[CHUTES] Initialized BERT classifier with model: {bert_model}")
            except Exception as e:
                logging.warning(f"[CHUTES] Failed to initialize BERT classifier: {e}")
                logging.warning("[CHUTES] Falling back to pattern-based routing")
        ...
        # Log provider info
        logging.info(f"=== Using CHUTES LLM provider ===")
        logging.info(f"  Default model: {model_name}")
        if self.use_routing:
            logging.info(f"  Routing enabled: {self.simple_model} (simple) / {self.complex_model} (complex)")
            logging.info(f"  Complexity threshold: {self.complexity_threshold}")
        else:
            logging.info(f"  Routing disabled - using default model")
        
        # Performance tracking
        self.performance_stats = {
            self.simple_model: {"calls": 0, "total_time": 0, "errors": 0},
            self.complex_model: {"calls": 0, "total_time": 0, "errors": 0},
        }
        
    def route_query(self, messages: LLMMessages) -> Dict[str, Any]:
        """Route query to appropriate model based on complexity"""
        # Extract the latest user query
        query = ""
        for message_list in reversed(messages):
        ...
        if not query:
            ...
            return {
                ...
                "routing_reason": "No query text found"
            }
        
        # Calculate complexity
        if self.classifier:
            try:
                complexity_score = self.classifier.get_banking_adjusted_complexity(query)
            except Exception as e:
                logging.warning(f"[CHUTES] Classifier error: {e}, using fallback")
                complexity_score = self._fallback_complexity_check(query)
        else:
            complexity_score = self._fallback_complexity_check(query)
        ...
        if complexity_score >= self.complexity_threshold:
            ...
            routing_reason = f"Complexity score {complexity_score:.3f} >= threshold {self.complexity_threshold}"
        else:
    ...
    def _fallback_complexity_check(self, query: str) -> float:
        """Simple pattern-based complexity check as fallback"""
        query_lower = query.lower()
        
        # Simple patterns
        simple_patterns = [
            "get", "show", "display", "what is", "find", "list",
            "customer id", "account number", "balance"
        ]
        
        # Complex patterns
        complex_patterns = [
            "analyze", "compare", "predict", "recommend", "assess",
            "evaluate", "strategy", "optimize", "trend", "pattern"
        ]
        ...
        if complex_count > simple_count:
            return 0.7 + (0.1 * min(complex_count, 3))
        elif simple_count > complex_count:
            return 0.3 - (0.1 * min(simple_count, 3))
        else:
            return 0.5
            
    def update_performance_stats(self, model: str, elapsed_time: float, success: bool):
        """Update performance statistics for a model"""
        if model not in self.performance_stats:
            self.performance_stats[model] = {"calls": 0, "total_time": 0, "errors": 0}
        ...
        if stats["calls"] % 10 == 0:
            ...
            logging.info(f"[CHUTES] Performance stats for {model}:")
            logging.info(f"  Calls: {stats['calls']}, Avg time: {avg_time:.2f}s, Error rate: {error_rate:.2%}")
    
    def adjust_threshold_based_on_performance(self):
        """Dynamically adjust routing threshold based on performance"""
        simple_stats = self.performance_stats.get(self.simple_model, {})
        complex_stats = self.performance_stats.get(self.complex_model, {})
        
        if simple_stats.get("calls", 0) < 10 or complex_stats.get("calls", 0) < 10:
            # Not enough data
            return
        
        simple_avg_time = simple_stats["total_time"] / simple_stats["calls"]
        complex_avg_time = complex_stats["total_time"] / complex_stats["calls"]
        simple_error_rate = simple_stats["errors"] / simple_stats["calls"]
        
        # Adjust threshold
        if simple_error_rate > 0.2:  # Too many errors with simple model
            self.complexity_threshold = max(0.3, self.complexity_threshold - 0.05)
            logging.info(f"[CHUTES] Lowered threshold to {self.complexity_threshold} due to high error rate")
        elif complex_avg_time > 60 and simple_avg_time < 15:  # Complex too slow
            self.complexity_threshold = min(0.7, self.complexity_threshold + 0.05)
            logging.info(f"[CHUTES] Raised threshold to {self.complexity_threshold} due to slow complex model")

    def _enhance_system_prompt(self, system_prompt: Any, enhancement: str) -> Any:
        """Safely enhance a system prompt with additional text."""
        if system_prompt is None:
            return enhancement
        elif isinstance(system_prompt, str):
            return system_prompt + enhancement
        elif isinstance(system_prompt, dict):
            # For dict format, create a new dict with enhanced content
            new_prompt = system_prompt.copy()
            if 'content' in new_prompt:
                if isinstance(new_prompt['content'], str):
                    new_prompt['content'] = new_prompt['content'] + enhancement
                else:
                    # If content is not a string, convert to string first
                    new_prompt['content'] = str(new_prompt['content']) + enhancement
            else:
                new_prompt['content'] = enhancement
            return new_prompt
        elif isinstance(system_prompt, list):
            # For list format (multimodal), append as new text item
            new_prompt = system_prompt.copy()
            new_prompt.append({"type": "text", "text": enhancement})
            return new_prompt
        else:
            # Fallback for any other type
            return str(system_prompt) + enhancement
    ...
    def generate(
        ...
        """Generate responses using Chutes OpenAI-compatible API with intelligent routing.
        ...
        if self.use_routing and _retry_count == 0:  # Only route on first attempt
            routing_result = self.route_query(messages)
            self.model_name = routing_result["model"]
            logging.info(f"[CHUTES ROUTING] Selected model: {self.model_name}")
            logging.info(f"[CHUTES ROUTING] Reason: {routing_result['routing_reason']}")
            logging.info(f"[CHUTES ROUTING] Complexity: {routing_result['complexity_score']:.3f} ({routing_result['complexity_type']})")
        ...
        if tools:
            if self.use_native_tool_calling:
                ...
                # Safely append to system prompt or create new one
                if openai_messages and openai_messages[0]["role"] == "system":
                    # Check the type of existing content
                    current_content = openai_messages[0]["content"]
                    
                    if isinstance(current_content, str):
                        # Simple string content - just concatenate
                        openai_messages[0]["content"] = current_content + tool_instructions
                    elif isinstance(current_content, list):
                        # Multimodal content list - append as new text item
                        openai_messages[0]["content"].append({
                            "type": "text",
                            "text": tool_instructions
                        })
                    elif isinstance(current_content, dict):
                        # Single dict content - convert to list format
                        openai_messages[0]["content"] = [
                            current_content,
                            {"type": "text", "text": tool_instructions}
                        ]
                    else:
                        # Unknown format - replace with tool instructions
                        logging.warning(f"[CHUTES] Unknown system content type: {type(current_content)}, replacing")
        ...
        self.update_performance_stats(self.model_name, elapsed_time, success)
        
        # Adjust threshold periodically
        if self.use_routing and sum(s["calls"] for s in self.performance_stats.values()) % 20 == 0:
        ...
        if not response:
            if _retry_count < 3:
                ...
                # Use the helper method to enhance the prompt
                if tools:
